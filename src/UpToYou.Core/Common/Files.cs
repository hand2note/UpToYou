...
Files {
    ...
    ClearDirectory(this string directory) {
        foreach (var file in directory.GetAllDirectoryFiles()) 
            File.Delete(file);
    ...
    public static int
    GetDirectoryFilesCount(this string dir) => dir.GetAllDirectoryFiles().Count();

    public static string? 
    FindFile(this string dir, Func<string, bool> predicate) => dir.GetAllDirectoryFiles().Where(predicate).FirstOrDefault();
